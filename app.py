...
# --- Team-based access control for all intern operations ---
# Helper: filter df for current tech lead (unless super admin)
def filter_df_by_team(df):
    if is_super_admin:
        return df
    return df[df["Team(eg :2 or 3)"].astype(str) == TEAM_NUMBERS.get(tech_lead, "")]

# Add/Update Intern
if page == "Add/Update Intern":
    st.title("Add or Update Intern")
    filtered_df = filter_df_by_team(df)
...
# Change History
    ...
    if os.path.exists(history_file):
        ...
        if not is_super_admin:
            hist_df = hist_df[hist_df["Intern"].isin(filter_df_by_team(df)["Name"])]
...
# --- End of team-based access control ---
...
# --- Refactored Project Teams tab for per-member progress tracking ---
    ...
    if "selected_team" in st.session_state and st.session_state.selected_team:
        ...
        if selected_team_name in teams:
            ...
            st.write(f"**Team Members:** {', '.join(str(m) for m in team_data['members'].keys())}")
